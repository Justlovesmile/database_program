...
def create_db(conn):
    ...
    c_sql=f"""
        ...
        create or replace view users_record_statistics as select user_id,nickname,count(posts.content) as '发帖数',count(comments.content) as '评论数' from users,posts,comments where user_id=posts.author_id and user_id=comments.author_id group by user_id;"""
    try:
        a=c_sql.split(';')
        for i in a[:-1]:
            i=i+';'
